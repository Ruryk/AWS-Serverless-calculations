# AWS-Serverless-calculations

This project creates an AWS Lambda function that accepts an image in `base64` format and converts it to `bmp`, `png`, or
`gif` format.

## Prerequisites

- AWS account
- AWS CLI installed and configured
- Postman or another tool for API testing

## Setup Instructions

### 1. Create the Project

- Create a project folder and initialize `npm`:

````bash
mkdir image-converter && cd image-converter
npm init -y
````

- Install the jimp dependency:

````bash
npm install jimp
````

- Create a file named index.mjs and paste the following code:

````javascript
import { Jimp, JimpMime } from 'jimp';

export const handler = async (event) => {
    try {
        const { image, format } = JSON.parse(event.body);

        if (!['bmp', 'png', 'gif'].includes(format)) {
            return {
                statusCode: 400,
                body: JSON.stringify({ message: "Invalid format specified." })
            };
        }

        const imageBuffer = Buffer.from(image, 'base64');
        const jimpImage = await Jimp.read(imageBuffer);

        let mimeType;
        switch (format) {
            case 'bmp':
                mimeType = JimpMime.bmp;
                break;
            case 'png':
                mimeType = JimpMime.png;
                break;
            case 'gif':
                mimeType = JimpMime.gif;
                break;
            default:
                mimeType = JimpMime.png;
                break;
        }

        const convertedBuffer = await jimpImage.getBuffer(mimeType);

        return {
            statusCode: 200,
            body: JSON.stringify({
                convertedImage: convertedBuffer.toString('base64'),
                message: `Image converted to ${ format } successfully.`
            })
        };
    } catch (error) {
        return {
            statusCode: 500,
            body: JSON.stringify({ message: "Error converting image", error: error.message })
        };
    }
};
````

### 2. Create a ZIP File for Lambda

- Ensure all dependencies are installed:

````bash
npm install
````

- Create a ZIP archive that includes index.mjs, package.json, and node_modules:

````bash
zip -r image-converter.zip index.mjs package.json node_modules/
````

### 3. Configure the Lambda Function

- Open the AWS Lambda Console.
- Click Create function and select Author from scratch.
- Provide a name (e.g., ImageConverter) and select Node.js 18.x\20.x as the runtime.
- Click Create function.
- In the Function code section, upload the image-converter.zip file.
- In the Handler field, enter index.handler.
- Under Configuration, go to Timeout and increase it to 30 seconds (or more if needed).
- Save the settings.

  ![functions](images/functions.png)
  ![function-overview](images/function-overview.png)
  ![function-code](images/function-code.png)

### 4. Set Up API Gateway

- Open the API Gateway Console and click Create API.
- Select HTTP API and click Build.
- Add an integration:
    - Choose Lambda and select your ImageConverter function.
- Create a route:
    - Set the method to POST and the route path to /convert.
- In Deployments, click Create and Deploy to complete the setup.
- Copy the API endpoint URL for testing.

  ![api](images/api.png)

### 5. Testing with Postman

- Open Postman and create a new request.
- Choose the POST method and enter the URL of your API Gateway with the /convert route.
- In the Headers section, add Content-Type: application/json.
- Enter the following format, where the inner JSON is stringifies to escape special characters:

````bash
{
  "image": "<YOUR_IMAGE_IN_BASE64>",
  "format": "png" // png | gif | bmp
}
````

- Replace <YOUR_IMAGE_IN_BASE64> with the actual Base64-encoded image string. This format ensures the JSON object is
  wrapped as a string and will be parsed correctly in Lambda.
- Ensure that the image and format data are enclosed in double quotes and the overall JSON structure is valid.
- Click Send and review the response. You should see a JSON object with the converted image in base64 format.

  ![test-postman](images/test-postman.png)
- Or you can create and run the test on the AWS side:
  ![test-aws](images/test-aws.png)

## Please Note:

- You can specify bmp, png, or gif as the format.
- Ensure that input files are within Lambda's size limits to avoid timeouts on large files.

## Decoding the Image

- For BMP (bmp):
  ![decode-bmp](images/decode-bmp.png)

- For PNG (png):
  ![decode-png](images/decode-png.png)

- For GIF (bmp):
  ![decode-gif](images/decode-gif.png)

## Payload Example

````json
{
  "body": "{\"image\": \"\", \"format\": \"png\"}"
}
````